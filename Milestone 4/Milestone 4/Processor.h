...
class Processor : public Task {
	...
	std::vector<CustomerOrder> waiting;  // orders to be processed
	std::vector<CustomerOrder> process;  // orders being processed
	std::vector<CustomerOrder> complete; // orders completed
...
public:
	Processor(const std::string&, std::function<Task::Quality(int)>);
	void load(Item&);
