...
Task::Task(const std::string& taskRecord): slots("1"){
	...
	//Instantiate a Utilities object for parsing...
	Utilities u;
	...
	//reset the tokens vector
	tokens.clear();

}

//validate - NEED TO REFACTOR FOR FUTURE VERSIONS
bool Task::validate(const Task& t) {
	...
	//Check again if non-empty fields are resolved or unresolved.
...
void Task::display(std::ostream& os) const {
	...
	os << std::setw(field_width) << std::left << "Task Name" << " : " << std::setw(field_width + 2) << "[" + name + "]" << " [" + slots + "]" << std::endl;

	std::string next[] = { "Continue to", "Redirect to" };
	for (int t = 0; t < 2; t++){
		if(!nextTask[t].empty()){
			os << std::setw(field_width) << std::right << next[t] << " : " << std::left << std::setw(field_width+2) << "[" + nextTask[t] + "]"; 
			if (pNextTask[t] == 0) { 
				os << " " + unvalidated; 
			} 
			os << std::endl;
